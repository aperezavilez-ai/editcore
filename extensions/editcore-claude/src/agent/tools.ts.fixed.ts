...
export const AGENT_TOOLS = [
  ...
  },
  {
    name: 'read_skill',
    description:
      'Carga el contenido completo de una skill disponible (ver catálogo en el system prompt). ' +
      'Úsala solo cuando la tarea actual coincida con la descripción de esa skill.',
    input_schema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Nombre exacto de la skill, tal como aparece en el catálogo.' },
      },
      required: ['name'],
    },
  },
...
async function execWriteFile(input: { path: string; content: unknown }): Promise<string> {
  if (typeof input.content !== 'string') {
    return (
      'ERROR: el parametro "content" debe ser un string de texto plano, no ' +
      `${typeof input.content === 'object' ? 'un objeto/JSON' : typeof input.content}. ` +
      'Si intentas escribir datos estructurados (JSON, config, etc.), conviertelos primero con ' +
      'JSON.stringify(objeto, null, 2) y pasa ese resultado como content.'
    );
  }
...
async function execApplyPatch(input: {
  ...
  if (typeof input.old_string !== 'string' || typeof input.new_string !== 'string') {
    return (
      'ERROR: "old_string" y "new_string" deben ser strings de texto plano. ' +
      'Si son datos estructurados, conviertelos primero con JSON.stringify().'
    );
  }
  const { checkSensitiveWrite } = await import('../autonomous/autonomousSecurity');
  const sensitive = checkSensitiveWrite(input.path);
...
async function showDiffAndConfirm(
  ...
  const choice = await vscode.window.showInformationMessage(
    ...
    'Aplicar',
    'Aplicar y confiar en el resto de la tarea',
    'Cancelar'
  );
  fs.promises.unlink(tmpPath).catch(() => {});

  if (choice === 'Aplicar y confiar en el resto de la tarea') {
    setTaskTrusted(true);
    const { appendAudit } = await import('../enterprise/orgConfig');
    await appendAudit({
      type: 'decision',
      kind: 'file_write',
      action: 'apply_and_trust',
      path: relativeLabel,
    });
    return true;
  }
...
export async function executeAgentTool(
  ...
  try {
    ...
    switch (name as BuiltinToolName) {
      ...
      case 'read_skill': {
        const { readSkillContent } = await import('./skillsCatalog');
        const content = await readSkillContent(input?.name);
        output = content ?? `Skill "${input?.name}" no encontrada en el catálogo disponible.`;
        break;
      }
      default:
