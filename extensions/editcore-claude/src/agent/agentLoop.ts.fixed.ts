...
import { buildSkillsCatalogPrompt } from "./skillsCatalog";
...
export async function runAgentTask(
  ...
  resetTaskTrust();
  const skillsCatalog = await buildSkillsCatalogPrompt();
  const baseWithSkills = skillsCatalog
    ? `${buildAgentSystemPromptBase()}\n\n${skillsCatalog}`
    : buildAgentSystemPromptBase();
  const systemPrompt = await buildSystemPrompt(baseWithSkills, roleId, customAgentId);
