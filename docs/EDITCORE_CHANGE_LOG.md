...
### 2026-06-30 — El Output channel del watcher no aparecía en el selector del build portable del usuario: diagnóstico movido a barra de estado + archivo en disco

- **Tipo de cambio**: instrumentación de diagnóstico real en `extensions/editcore-claude`, sin nuevas dependencias.
- **Contexto**: tras la entrada anterior de este mismo día (Output channel "EditCore: Dev Server Watcher"), el usuario confirmó con capturas reales que la extensión sí se activa de cero, sin errores, en el log de "Extension Host" (`ExtensionService#_doActivateExtension editcore.editcore-claude, startup: true, activationEvent: 'onStartupFinished'`) — pero el canal de Output nunca apareció en el selector desplegable del panel Output de su build portable, incluso después de confirmar que la activación era fresca (no un log viejo en caché).
- **Causa probable (no 100% confirmada, pero consistente con el resto del repo)**: el build portable del usuario tiene una cantidad importante de parches sobre la UI de chat/Output (`patch-portable-chat-clean-ui.js`, `patch-portable-chat-browser-context.js`, etc., visibles en `scripts/deploy-extensions-to-portable.ps1`); es plausible que alguno de esos parches filtre o limite qué canales de Output se listan. En vez de seguir intentando diagnosticar a través de una UI que ya demostró no ser confiable en este entorno, se movió el diagnóstico a dos vías que no dependen de ningún render de UI: una entrada de barra de estado siempre visible y un archivo de texto plano en disco.
- **Archivos modificados**:
  - `extensions/editcore-claude/src/preview/devServerWatcher.ts`: se agregó una función `trace()` que escribe el mismo mensaje en 3 lugares a la vez — el Output channel existente (se mantiene por si sí funciona), un `vscode.StatusBarItem` visible en la barra inferior con el estado actual del watcher (registrado, vigilando, puerto detectado, abriendo browser), y un archivo `editcore-dev-watcher.log` que se crea directamente en la raíz del proyecto abierto (ej. `D:\PROGRAMAS IA\CONNECTXI\editcore-dev-watcher.log`), usando `fs.appendFileSync` — append directo a disco, sin pasar por ninguna API de UI de VS Code/Code-OSS que pueda estar parcheada o filtrada.
- **Pruebas**: `npx tsc --noEmit -p extensions/editcore-claude` → sin errores; `npm run compile` → sin errores.
- **Validación**: pendiente. El usuario debe hacer `git pull` + `deploy-extensions-to-portable.ps1` + recargar EditCore, correr `npm run dev`, y revisar: (1) la barra de estado inferior de EditCore (debería decir "EditCore: ..." apenas se abra el proyecto), y (2) el archivo `editcore-dev-watcher.log` que debería aparecer en la raíz de su proyecto Conectxi (visible en el Explorer del propio EditCore) con el detalle real de cada paso del watcher.
- **Nota de higiene**: ese archivo `.log` se crea dentro del proyecto del usuario (Conectxi), no de este repositorio — es responsabilidad del usuario (o de un futuro cambio) agregarlo a `.gitignore` de Conectxi si no quiere versionarlo ahí. No se tocó ningún `.gitignore` de este repositorio porque el archivo nunca se crea dentro de `editcore`.

### 2026-06-30 — Fix cosmético en `deploy-extensions-to-portable.ps1`: stderr informativo del generador de iconos se imprimía como error rojo de PowerShell

- **Tipo de cambio**: fix en script de deploy, sin cambios funcionales.
- **Causa raíz**: `scripts/generate-editcore-icons.py` invoca `scripts/update-product-checksums.js` (Node), que escribe avisos informativos (`omitido (no existe): ...`) por stderr. PowerShell renderiza cualquier línea de stderr de un comando nativo canalizada a `Out-Host` como un `NativeCommandError` en rojo, sin importar `$ErrorActionPreference = "Continue"`. El propio script ya tenía este mismo arreglo aplicado para `npm` en `Invoke-Npm`, pero no para la llamada a `python`.
- **Archivo modificado**: `scripts/deploy-extensions-to-portable.ps1` — cambiado `python $genIcons 2>&1 | Out-Host` por `python $genIcons 2>&1 | ForEach-Object { Write-Host $_ }` (mismo patrón que `Invoke-Npm`).
- **Commit**: `385aa8d`.

### 2026-06-30 — Causa raíz real del bug del auto-open de browser: archivos sueltos sin trackear rompían la compilación del usuario

- **Tipo de cambio**: diagnóstico + resolución colaborativa con el usuario (no requirió cambios de código en el repo).
- **Causa raíz encontrada**: el checkout local del usuario (`D:\EDITCORE`) tenía `extensions/editcore-claude/src/company/` y `extensions/editcore-claude/test/company-utils.test.js` como archivos **untracked** (nunca comiteados, confirmados ausentes en todo el historial de git de este repositorio). Como `extensions/editcore-claude/tsconfig.json` no tiene `include` (compila todo bajo `src/`), esos archivos sueltos —que importaban módulos inexistentes `../aos/agentRegistry` y `../ecosystem/enterpriseSecurity`— rompían `npm run compile` con `TS2307`/`TS7006`, abortando todo `deploy-extensions-to-portable.ps1` antes de generar `out/preview/devServerWatcher.js`. Compuesto además por: el usuario había quedado atascado en `main` por `git pull` sin `git checkout` previo, y por un `git checkout` bloqueado silenciosamente por cambios locales sin commitear (resuelto con `git stash`).
- **Resolución**: el usuario borró `src/company/` y el test huérfano de su checkout local (no eran parte del repo), volvió a correr el deploy, y confirmó que `devServerWatcher.js` se generó y que el watcher funciona en su entorno real — Output channel visible, barra de estado mostrando el sondeo de puertos, y el browser integrado abriéndose solo al levantar `npm run dev` en su proyecto Conectxi.
- **Estado**: la feature de auto-open de browser (`devServerWatcher.ts`, instrumentada en las dos entradas anteriores de este changelog) queda **confirmada funcionando en el entorno real del usuario**, cerrando el ciclo de troubleshooting de este día.

### 2026-06-30 — Roadmap de Prioridad 1-3 (`EDITCORE_MASTER_ROADMAP.md`): tests, CI, convenciones de API, integración LLM real y ejecución de orquestación

- **Tipo de cambio**: feature + testing + CI, sin nuevas dependencias externas (usa `node --test` nativo de Node 22, ya usado por las extensiones).
- **P1.1 — Suite de tests para `lib/`**: nuevos `lib/__tests__/{aiGovernance,modelRouter,taskReasoning}.test.js` (24 tests), siguiendo el mismo patrón de lógica duplicada en JS puro que ya usan los tests de `extensions/editcore-claude/test/*.test.js` (evita un paso de compilación solo para testear). Script `test` agregado a `package.json` raíz.
  - **Bug real encontrado y corregido**: en `lib/aiGovernance.ts`, la rama que permite una acción sin aprobación (`autonomyLevel >= rule.min_autonomy_level && !rule.requires_approval`) no verificaba `rule.allowed`. Para una acción con `allowed: false` (ej. `modify_secrets`, con `min_autonomy_level: 99` pensado como inalcanzable), si alguna vez `autonomyLevel` llegara a ese mínimo, la función devolvía `allowed: true` contradiciendo la regla base. Hoy es inalcanzable en la práctica (los niveles de autonomía van de 1 a 5), pero es un fallo de lógica real, no cosmético. Fix: se agregó `rule.allowed &&` a la condición.
- **P1.2 — CI cubre `api/`/`lib/`**: nuevo job `web-backend` en `.github/workflows/editcore-ci.yml` que corre `npm run typecheck` (`tsc --noEmit`) y `npm test` sobre la raíz del repo en cada push/PR — antes el CI solo cubría las extensiones del IDE.
- **P2.1 — Convención de rutas `/v1`**: nuevo `docs/EDITCORE_API_CONVENTIONS.md` documentando que toda ruta nueva va en `api/v1/<módulo>/` y que las rutas legacy (`api/{auth,community,developer,evolution,org,usage}`) se mantienen sin expandirse.
- **P3.1 — Integración real de LLM**: nuevo `lib/llmClient.ts` (`isLlmConfigured()`, `generateReasoning()`) que llama a la API de Claude usando `process.env.ANTHROPIC_API_KEY` — **el usuario debe configurar esa variable de entorno en su propio dashboard de Vercel; nunca se escribió ni se referenció ningún valor de API key en este repositorio**. `api/v1/aios/orchestrate.ts` ahora usa razonamiento real del LLM para enriquecer `strategy` cuando la key está configurada, con fallback automático y silencioso a la estrategia de reglas fijas de `decomposeGoal` si no lo está o si la llamada falla — el endpoint nunca se rompe por falta de LLM.
- **P3.2 — Ejecución de planes de orquestación**: nuevo endpoint `POST /api/v1/aios/execute.ts`. Avanza un run un paso a la vez; si la siguiente subtarea tiene `requires_human_approval: true`, el run pasa a `status: 'paused'` (valor ya existente en el `check` constraint de `ai_orchestration_runs`, sin migración nueva) y no avanza hasta recibir `{ approved: true }` en una nueva petición — aplica el Trust Framework por paso, no solo al crear el run. Cada paso genera un resultado de razonamiento/análisis textual (vía LLM real si está configurado, o un resumen de reglas fijas si no) — **no ejecuta acciones destructivas reales** (no escribe archivos ni deploya), consistente con el riesgo "alto" que el propio roadmap le asignaba a esta tarea.
